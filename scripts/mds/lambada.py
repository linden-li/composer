...
"""LAMBADA streaming dataset conversion scripts."""
...
def get(split: str) -> IterableDataset:
    ...
    class ShardedLAMBADA(IterableDataset):

        def __init__(self):
            self.dataset = datasets.load_dataset(
                path='lambada', name='plain_text', split=split, streaming=True)
...
def main(args: Namespace) -> None:
    """Main: create LAMBADA streaming dataset.
    ...
    for (split, split_new_name, expected_num_samples) in [
        ...
        with StreamingDatasetWriter(dirname=os.path.join(args.out_root, split_new_name),
            ...
            out.write_samples(samples=each(dataset), use_tqdm=bool(args.tqdm))
